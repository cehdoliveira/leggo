...
class banners_controller
{
    ...
    private function filter($info)
    {
        ...
        if (isset($info["get"]["filter_published"]) && !empty($info["get"]["filter_published"])) {
            $done["filter_published"] = $info["get"]["filter_published"];
            $filter["filter_published"] = " status_published = '" . $info["get"]["filter_published"] . "' ";
        }
        if (isset($info["get"]["filter_profile"]) && !empty($info["get"]["filter_profile"])) {
            $done["filter_profile"] = $info["get"]["filter_profile"];
            $filter["filter_profile"] = " idx in ( select banners_id from banners_profiles where active = 'yes' and profiles_id = '" . $info["get"]["filter_profile"] . "' ) ";
        }
    ...
    public function display($info)
    {
        ...
        $banners->set_order(array(" idx asc "));

        if ($info["format"] == ".html") {
            $banners->set_paginate(array((int)$info["sr"] > $paginate ? (int)$info["sr"] : 0, $paginate));
        } else {
        ...
        $banners->attach(array("profiles"), false, ' and idx > 1 ');
        $data = $banners->data;
        ...
        switch ($info["format"]) {
            case ".json":
                ...
                echo json_encode(
                    array(
                        "total" => array_merge(array("total" => array_sum($t)), $t),
                        "row" => $data
                    )
                );
                break;
    ...
    public function save($info)
    {
        ...
        if (isset($_FILES["image_mobile"]) && is_file($_FILES["image_mobile"]["tmp_name"])) {
            $d = preg_split("/\./", $_FILES["image_mobile"]["name"]);
            $extension = $d[count($d) - 1];
            $name = generate_slug(preg_replace("/\." . $extension . "$/", "", $_FILES["image_mobile"]["name"]));
            $extension = date("YmdHis") . "." . $extension;
            $file = "furniture/upload/topbanner/mobile/" . $name . $extension;
            ...
            move_uploaded_file($_FILES["image_mobile"]["tmp_name"], constant("cRootServer") . $file);
            $info["post"]["img_mobile"] = $file;
        }
