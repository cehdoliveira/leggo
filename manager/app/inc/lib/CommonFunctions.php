...
function assign_subscription_profile(int $userId, string $plan): void
{
  ...
  // Remover perfis de assinatura anteriores — nunca toca quarterly
...
function invalidate_quarterly_cache(int $userId): void
{
  ...
  if ($redis) {
    try {
      $redis->del("quarterly_access:{$userId}");
    } catch (\Exception $e) {
